	...
	for(int i = 0; i < listChanelType.size(); i++){
		...
		for(int iChanel = 0; iChanel < listChanel.size(); iChanel++){
		    ...
		    //String parameter = "?"+FrmChanel.fieldNames[FrmChanel.FRM_FIELD_CHANEL_ID]+"="+chanel.getOID();
                    String dataPopulation = " data-namachanel='"+chanel.getChanelName()+"' data-"+FrmChanel.fieldNames[FrmChanel.FRM_FIELD_CHANEL_ID]+"='"+chanel.getOID()+"'";
		    content+="<li class='dropdown'><a style='cursor:pointer' "+dataPopulation+" class='dropdown-toggle chanel' data-toggle='dropdown'>"+chanel.getChanelName()+"  <span class='caret'></span></a>";
			...
			for(int iGroup = 0; iGroup < listGroup.size(); iGroup++){
			    AreaType areaType = (AreaType) listGroup.get(iGroup);
                            String dataPopulation2 = " data-namagroup='"+areaType.getAreaTypeName()+"'  data-"+FrmAreaType.fieldNames[FrmAreaType.FRM_FIELD_AREA_TYPE_ID]+"='"+areaType.getOID()+"'";
			    //String parameter2 = "&"+FrmAreaType.fieldNames[FrmAreaType.FRM_FIELD_AREA_TYPE_ID]+"="+areaType.getOID();
			    content += ""
			    + "<li>"
				+ "<a style='cursor:pointer' class='trigger reportfilter' data-for='group' "+dataPopulation+" "+dataPopulation2+">"+areaType.getAreaTypeName()+" <span class='right-caret'></span></a>";
				...
				if(listCategory.size() > 0){
					...
					AreaUsage areaUsage = (AreaUsage) listCategory.get(iCategory);
                                        String dataPopulation3 = " data-namacategory='"+areaUsage.getAreaUsageName()+"' data-"+FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_ID]+"='"+areaUsage.getOID()+"'";
					//String parameter3 = "&"+FrmAreaUsage.fieldNames[FrmAreaUsage.FRM_FIELD_AREA_USAGE_ID]+"="+areaUsage.getOID();
					content += ""
					+ "<li>"
					    + "<a style='cursor:pointer' "+dataPopulation+" "+dataPopulation2+" "+dataPopulation3+"  data-for='category' class='trigger reportfilter'>"+areaUsage.getAreaUsageName()+" <span class='right-caret'></span></a>";
...
<header class="main-header">
  <nav class="navbar navbar-static-top">
	...
	<div class="col-xs-5">
		...
		<a href="javascript:" class="navbar-brand"><b><%= topLeftHeader %></b></a>
	    </div>
